...
export const gstErrorList = Object.freeze({
    "100": "Invalid Json",
...
export function getGstErrorMessage(codeString) {
    const codes = codeString.split(",").map(code => code.trim());
    const messages = codes.map(code => gstErrorList[code] || `Unknown error code: ${code}`);
    return messages;
}