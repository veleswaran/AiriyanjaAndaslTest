...
export class QuickSpecSelecter extends React.Component {
    ...
    onSelect = (data) => {
        ...
        for (const ex of allLis) {
            if (id === ex.specGroup?.id) { canAdd = false; break; }
        }
        if (canAdd) {
    ...
    setValue = (specs = []) => {
        ...
        const seen = new Set();
        specs.forEach(spec => {
            const { specsGroupId } = spec;
            if (!seen.has(specsGroupId)) {
                seen.add(specsGroupId);
    ...
    render() {
        ...
        return (<>
            <thead key={uuid + "th"} className={head}>
                <tr>
                    ...
                    <th className="text-end">
                        <PickerButton disabled={disabled} css="btn btn-sm btn-outline-primary bi bi-plus" searchComponent={SearchSpecsName} onSelect={this.onSelect} modalProps={{ size: "xl", css: { footer: "visually-hidden" } }} title="Select specs name" />
                    </th>
                </tr>
            ...
            <tbody key={uuid + "tb"} className={body}>
                <MultiRender ref={this.multiRef} onUpdate={this.onUpdate} component={QuickSpecsAdd} />
            </tbody>
        </>);
    }
}
